from market import db

class User(db.Model):
    id=db.Column(db.Integer(),primary_key=True)
    username=db.Column(db.String(length=30),unique=True,nullable=False)
    email_address=db.Column(db.Integer(),nullable=False)
    password_hash=db.Column(db.String(length=12),unique=True,nullable=False)
    budget=db.Column(db.String(length=1040),nullable=False,default=1000)
    items=db.relationship("Item",backref="owner_name",lazy=True)

   


class Item(db.Model):
    id=db.Column(db.Integer(),primary_key=True)
    name=db.Column(db.String(length=30),unique=True,nullable=False)
    price=db.Column(db.Integer(),nullable=False)
    barcode=db.Column(db.String(length=12),unique=True,nullable=False)
    description=db.Column(db.String(length=1040),unique=True,nullable=False)
    owner=db.Column(db.Integer(),db.ForeignKey("user.id"))
    def __repr__(self):
        return f"username:{self.name },price{self.price },barcode:{self.barcode }"